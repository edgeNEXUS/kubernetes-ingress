...
import (
	...
	"github.com/edgeNEXUS/kubernetes-ingress/pkg/apis/configuration/validation"
	k8s_edge "github.com/edgeNEXUS/kubernetes-ingress/pkg/client/clientset/versioned"
	k8s_edge_informers "github.com/edgeNEXUS/kubernetes-ingress/pkg/client/informers/externalversions"
...
const (
	...
	IngressControllerName = "edgenexus.io/ingress-controller"
)
...
type podEndpoint struct {
	Address string
	PodName string
	// MeshPodOwner is used for Edgenexus Service Mesh metrics
	configs.MeshPodOwner
}

// LoadBalancerController watches Kubernetes API and
// reconfigures Edgenexus via EdgeController when needed
type LoadBalancerController struct {
	client                        kubernetes.Interface
	confClient                    k8s_edge.Interface
	dynClient                     dynamic.Interface
	cacheSyncs                    []cache.InformerSynced
	sharedInformerFactory         informers.SharedInformerFactory
	confSharedInformerFactorry    k8s_edge_informers.SharedInformerFactory
...
func NewLoadBalancerController(input NewLoadBalancerControllerInput) *LoadBalancerController {
	...
	lbc.recorder = eventBroadcaster.NewRecorder(scheme.Scheme,
		api_v1.EventSource{Component: "edgenexus-ingress-controller"})
	...
	glog.V(3).Infof("Edgenexus Ingress Controller has class: %v", input.IngressClass)
	...
	if lbc.areCustomResourcesEnabled {
		lbc.confSharedInformerFactorry = k8s_edge_informers.NewSharedInformerFactoryWithOptions(lbc.confClient, input.ResyncPeriod, k8s_edge_informers.WithNamespace(lbc.namespace))
	...
	if input.ConfigMaps != "" {
		edgeConfigMapsNS, edgeConfigMapsName, err := ParseNamespaceName(input.ConfigMaps)
		if err != nil {
			glog.Warning(err)
		} else {
			lbc.watchEdgeConfigMaps = true
			lbc.addConfigMapHandler(createConfigMapHandlers(lbc, edgeConfigMapsName), edgeConfigMapsNS)
		}
	}
...
func (lbc *LoadBalancerController) updateAllConfigs() {
	...
	if lbc.configMap != nil {
		cfgParams = configs.ParseConfigMap(lbc.configMap, lbc.isEdgePlus, lbc.appProtectEnabled)
	}
...
// it will report warnings. (See https://github.com/edgeNEXUS/kubernetes-ingress/issues/1448 )
...
func (lbc *LoadBalancerController) sync(task task) {
	...
	if !lbc.isEdgeReady && lbc.syncQueue.Len() == 0 {
		lbc.configurator.EnableReloads()
		lbc.updateAllConfigs()

		lbc.isEdgeReady = true
		glog.V(3).Infof("Edgenexus is ready")
	}
}
...
func (lbc *LoadBalancerController) syncPolicy(task task) {
	...
	if polExists && lbc.HasCorrectIngressClass(obj) {
		...
		err := validation.ValidatePolicy(pol, lbc.isEdgePlus, lbc.enablePreviewPolicies, lbc.appProtectEnabled)
		if err != nil {
...
// Such changes need to be processed at once to prevent any inconsistencies in the generated Edgenexus config.
...
func (lbc *LoadBalancerController) createIngressEx(ing *networking.Ingress, validHosts map[string]bool, validMinionPaths map[string]bool) *configs.IngressEx {
	...
	if ing.Spec.DefaultBackend != nil {
		...
		if err != nil {
			...
			if err == nil && external && lbc.isEdgePlus {
		...
		if lbc.isEdgePlus && lbc.isHealthCheckEnabled(ing) {
		...
		if (lbc.isEdgePlus && lbc.isPrometheusEnabled) || lbc.isLatencyMetricsEnabled {
	...
	for _, rule := range ing.Spec.Rules {
		...
		for _, path := range rule.HTTP.Paths {
			...
			if lbc.isEdgePlus || lbc.isLatencyMetricsEnabled {
				for _, endpoint := range podEndps {
					ingEx.PodsByIP[endpoint.Address] = configs.PodInfo{
						Name:         endpoint.PodName,
						MeshPodOwner: endpoint.MeshPodOwner,
...
func (lbc *LoadBalancerController) getPolicies(policies []conf_v1.PolicyReference, ownerNamespace string) ([]*conf_v1.Policy, []error) {
	...
	for _, p := range policies {
		...
		err = validation.ValidatePolicy(policy, lbc.isEdgePlus, lbc.enablePreviewPolicies, lbc.appProtectEnabled)
		if err != nil {
...
func (lbc *LoadBalancerController) createTransportServerEx(transportServer *conf_v1alpha1.TransportServer, listenerPort int) *configs.TransportServerEx {
	...
	for _, u := range transportServer.Spec.Upstreams {
		...
		if lbc.isEdgePlus && lbc.isPrometheusEnabled {
			for _, endpoint := range podEndps {
...
func (lbc *LoadBalancerController) getEndpointsForIngressBackend(backend *networking.IngressBackend, svc *api_v1.Service) (result []podEndpoint, isExternal bool, err error) {
	...
	if err != nil {
		if svc.Spec.Type == api_v1.ServiceTypeExternalName {
			if !lbc.isEdgePlus {
				return nil, false, fmt.Errorf("Type ExternalName Services feature is only available in 'Edgenexus +'")
			}
...
func (lbc *LoadBalancerController) isHealthCheckEnabled(ing *networking.Ingress) bool {
	if healthCheckEnabled, exists, err := configs.GetMapKeyAsBool(ing.Annotations, "edgenexus.io/health-checks", ing); exists {
		if err != nil {
			glog.Error(err)
		}
...
// IsEdgeReady returns ready status of Edgenexus
func (lbc *LoadBalancerController) IsEdgeReady() bool {
	return lbc.isEdgeReady
}
