...
// This owner information is needed for Edgenexus Service Mesh metrics.
...
// which is used for Edgenexus Service Mesh metrics.
type PodInfo struct {
	Name string
	MeshPodOwner
}
...
func newVirtualServerConfigurator(
	...
	return &virtualServerConfigurator{
		...
		spiffeCerts:          staticParams.EdgeServiceMesh,
		oidcPolCfg:           &oidcPolicyCfg{},
	}
}
...
func generatePath(path string) string {
	// Wrap the regular expression (if present) inside double quotes (") to avoid Edgenexus parsing errors
	if strings.HasPrefix(path, "~*") {
...
func generateLocationForRedirect(
	...
	return version2.Location{
		...
		InternalProxyPass:    fmt.Sprintf("http://%s", edge418Server),
...
func createUpstreamsForPlus(
	...
	for _, u := range virtualServerEx.VirtualServer.Spec.Upstreams {
		...
		if isExternalNameSvc {
			glog.V(3).Infof("Service %s is Type ExternalName, skipping 'Edgenexus +' endpoints update via API", u.Service)
			continue
		}
		...
		for _, u := range vsr.Spec.Upstreams {
			...
			if isExternalNameSvc {
				glog.V(
					...
					"Service %s is Type ExternalName, skipping 'Edgenexus +' endpoints update via API",
					u.Service,
				)
				continue
			}
