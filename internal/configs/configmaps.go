...
func ParseConfigMap(cfgm *v1.ConfigMap, edgePlus bool, hasAppProtect bool) *ConfigParams {
	...
	if resolverAddresses, exists, err := GetMapKeyAsStringSlice(cfgm.Data, "resolver-addresses", cfgm, ","); exists {
		if err != nil {
			...
			if edgePlus {
				...
				glog.Warning("ConfigMap key 'resolver-addresses' requires 'Edgenexus +'")
			}
		}
	}
	...
	if resolverIpv6, exists, err := GetMapKeyAsBool(cfgm.Data, "resolver-ipv6", cfgm); exists {
		if err != nil {
			...
			if edgePlus {
				...
				glog.Warning("ConfigMap key 'resolver-ipv6' requires 'Edgenexus +'")
			}
		}
	}
	...
	if resolverValid, exists := cfgm.Data["resolver-valid"]; exists {
		if edgePlus {
			...
			glog.Warning("ConfigMap key 'resolver-valid' requires 'Edgenexus +'")
		}
	}
	...
	if resolverTimeout, exists := cfgm.Data["resolver-timeout"]; exists {
		if edgePlus {
			...
			glog.Warning("ConfigMap key 'resolver-timeout' requires 'Edgenexus +'")
		}
	}
...
// GenerateEdgeMainConfig generates MainConfig.
func GenerateEdgeMainConfig(staticCfgParams *StaticConfigParams, config *ConfigParams) *version1.MainConfig {
	edgeCfg := &version1.MainConfig{
		...
		EdgeStatus:                         staticCfgParams.EdgeStatus,
		EdgeStatusAllowCIDRs:               staticCfgParams.EdgeStatusAllowCIDRs,
		EdgeStatusPort:                     staticCfgParams.EdgeStatusPort,
