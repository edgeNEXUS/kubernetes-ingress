...
func generateEdgeCfg(ingEx *IngressEx, apResources AppProtectResources, isMinion bool, baseCfgParams *ConfigParams, isPlus bool,
	isResolverConfigured bool, staticParams *StaticConfigParams, isWildcardEnabled bool) (version1.IngressEdgeConfig, Warnings) {
	...
	if len(grpcServices) > 0 && !cfgParams.HTTP2 {
		glog.Errorf("Ingress %s/%s: annotation edgenexus.io/grpc-services requires HTTP2, ignoring", ingEx.Ingress.Namespace, ingEx.Ingress.Name)
	...
	return version1.IngressEdgeConfig{
		...
		SpiffeClientCerts: staticParams.EdgeServiceMesh && !cfgParams.SpiffeServerCerts,
	}, allWarnings
}
...
func generateJWTConfig(owner runtime.Object, secretRefs map[string]*secrets.SecretReference, cfgParams *ConfigParams,
	...
	// For (1) and (2), 'Edgenexus +' will reject such a key at runtime and return 500 to clients.
...
// Mandatory Health Checks can cause Edgenexus to return errors on reload, since all Upstreams start
...
func createUpstream(ingEx *IngressEx, name string, backend *networking.IngressBackend, stickyCookie string, cfg *ConfigParams,
	...
	if exists {
		...
		// Always false for Edgenexus OSS
...
func generateEdgeCfgForMergeableIngresses(mergeableIngs *MergeableIngresses, masterApResources AppProtectResources,
	baseCfgParams *ConfigParams, isPlus bool, isResolverConfigured bool, staticParams *StaticConfigParams,
	isWildcardEnabled bool) (version1.IngressEdgeConfig, Warnings) {
	...
	if len(removedAnnotations) != 0 {
		glog.Errorf("Ingress Resource %v/%v with the annotation 'edgenexus.io/mergeable-ingress-type' set to 'master' cannot contain the '%v' annotation(s). They will be ignored",
			mergeableIngs.Master.Ingress.Namespace, mergeableIngs.Master.Ingress.Name, strings.Join(removedAnnotations, ","))
	}
	isMinion := false

	masterEdgeCfg, warnings := generateEdgeCfg(mergeableIngs.Master, masterApResources, isMinion, baseCfgParams, isPlus, isResolverConfigured, staticParams, isWildcardEnabled)
	...
	masterServer = masterEdgeCfg.Servers[0]
	masterServer.Locations = []version1.Location{}

	upstreams = append(upstreams, masterEdgeCfg.Upstreams...)

	if masterEdgeCfg.Keepalive != "" {
		keepalive = masterEdgeCfg.Keepalive
	}
	...
	for _, minion := range minions {
		...
		if len(removedAnnotations) != 0 {
			glog.Errorf("Ingress Resource %v/%v with the annotation 'edgenexus.io/mergeable-ingress-type' set to 'minion' cannot contain the %v annotation(s). They will be ignored",
		...
		edgeCfg, minionWarnings := generateEdgeCfg(minion, dummyApResources, isMinion, baseCfgParams, isPlus, isResolverConfigured, staticParams, isWildcardEnabled)
		...
		for _, server := range edgeCfg.Servers {
			for _, loc := range server.Locations {
				loc.MinionIngress = &edgeCfg.Ingress
				locations = append(locations, loc)
			}
		...
		upstreams = append(upstreams, edgeCfg.Upstreams...)
	}
	...
	return version1.IngressEdgeConfig{
		...
		Upstreams:         upstreams,
		Keepalive:         keepalive,
		Ingress:           masterEdgeCfg.Ingress,
		SpiffeClientCerts: staticParams.EdgeServiceMesh && !baseCfgParams.SpiffeServerCerts,
	}, warnings
}

func isSSLEnabled(isSSLService bool, cfgParams ConfigParams, staticCfgParams *StaticConfigParams) bool {
	return isSSLService || staticCfgParams.EdgeServiceMesh && !cfgParams.SpiffeServerCerts
}
