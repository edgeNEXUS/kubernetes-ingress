...
func NewLatencyMetricsCollector(
	...
	return &LatencyMetricsCollector{
		httpLatency: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			...
			Help:        "Bucketed response times from when Edgenexus establishes a connection to an upstream server to when the last byte of the response body is received by Edgenexus",
...
func parseMessage(msg string) (latencyMetric, error) {
	msgParts := strings.Split(msg, edgeSeparator)
	if len(msgParts) != 2 {
		return latencyMetric{}, fmt.Errorf("wrong message format: %s, expected message to start with \"%s\"", msg, edgeSeparator)
	}
	var sm syslogMsg
	info := msgParts[1]
	...
	if sm.UpstreamAddr == sm.ProxyHost {
		...
		return latencyMetric{}, fmt.Errorf("Edgenexus could not connect to upstream")
	}
