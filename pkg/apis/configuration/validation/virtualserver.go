...
// The rules for Edgenexus upstream names are less strict than IsDNS1035Label.
...
// returnBodyVariables includes Edgenexus variables allowed to be used in a return body.
...
// validRedirectVariableNames includes Edgenexus variables allowed to be used in redirects.
...
var edgeVariableRegexp = regexp.MustCompile(`\$\{([^}]*)\}`)
...
func captureVariables(s string) []string {
	...
	res := edgeVariableRegexp.FindAllStringSubmatch(s, -1)
	for _, n := range res {
...
func (vsv *VirtualServerValidator) validateRedirectURL(redirectURL string, fieldPath *field.Path, validVars map[string]bool) field.ErrorList {
	...
	if !escapedStringsFmtRegexp.MatchString(redirectURL) {
		msg := validation.RegexError(escapedStringsErrMsg, escapedStringsFmt, "http://www.edgenexus.io", "${scheme}://${host}/green/", `\"http://www.edgenexus.io\"`)
...
// We support prefix-based Edgenexus locations, positive case-sensitive/insensitive regular expressions matches and exact matches.
// More info http://edgenexus.io/en/docs/http/ngx_http_core_module.html#location
...
// validVariableNames includes Edgenexus variables allowed to be used in conditions.
// Not all Edgenexus variables are allowed. The full list of Edgenexus variables is at https://edgenexus.io/en/docs/varindex.html
...
func validateVariableName(name string, fieldPath *field.Path) field.ErrorList {
	...
	if _, exists := validVariableNames[name]; !exists {
		return append(allErrs, field.Invalid(fieldPath, name, "is not allowed or is not an Edgenexus variable"))
	}
...
func rejectPlusResourcesInOSS(upstream v1.Upstream, idxPath *field.Path, isPlus bool) field.ErrorList {
	...
	if upstream.HealthCheck != nil {
		allErrs = append(allErrs, field.Forbidden(idxPath.Child("healthCheck"), "active health checks are not supported in Edgenexus"))
	}

	if upstream.SlowStart != "" {
		allErrs = append(allErrs, field.Forbidden(idxPath.Child("slow-start"), "slow start is not supported in Edgenexus"))
	}

	if upstream.SessionCookie != nil {
		allErrs = append(allErrs, field.Forbidden(idxPath.Child("sessionCookie"), "sticky cookies are not supported in Edgenexus"))
	}

	if upstream.Queue != nil {
		allErrs = append(allErrs, field.Forbidden(idxPath.Child("queue"), "queue is not supported in Edgenexus"))
	}

	if upstream.NTLM {
		allErrs = append(allErrs, field.Forbidden(idxPath.Child("ntlm"), "NTLM is not supported in Edgenexus"))
	}
