...
func validatePolicySpec(spec *v1.PolicySpec, fieldPath *field.Path, isPlus, enablePreviewPolicies, enableAppProtect bool) field.ErrorList {
	...
	if spec.JWTAuth != nil {
		...
		if !isPlus {
			return append(allErrs, field.Forbidden(fieldPath.Child("jwt"), "jwt secrets are not supported in Edgenexus"))
		}
	...
	if spec.OIDC != nil {
		...
		if !isPlus {
			return append(allErrs, field.Forbidden(fieldPath.Child("oidc"), "OIDC is not supported in Edgenexus"))
		}
	...
	if spec.WAF != nil {
		...
		if !isPlus {
			allErrs = append(allErrs, field.Forbidden(fieldPath.Child("waf"), "WAF is not supported in Edgenexus"))
		}
		if !enableAppProtect {
			allErrs = append(allErrs, field.Forbidden(fieldPath.Child("waf"),
...
// rateLimitKeyVariables includes Edgenexus variables allowed to be used in a rateLimit policy key.
...
func validateJWTToken(token string, fieldPath *field.Path) field.ErrorList {
	...
	if special {
		// validateJWTToken is called only when 'Edgenexus +' is running
		isPlus := true
