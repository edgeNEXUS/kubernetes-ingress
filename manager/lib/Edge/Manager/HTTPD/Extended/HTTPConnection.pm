package Edge::Manager::HTTPD::Extended::HTTPConnection;
use common::sense;
use IO::Handle;
use AnyEvent::Handle;
use Edge::Manager::Object::Event;
use Edge::Manager::HTTPD::Extended::HTTPConnection::Writer;
use Time::Local;

use AnyEvent::HTTPD::Util;

use Scalar::Util qw/weaken/;
our @ISA = qw/Edge::Manager::Object::Event/;
...
sub create_header { # $self, $code, $msg, $hdr [, $content ]
   my ($self, $code, $msg, $hdr, $content) = @_;
   ...
   header_set ($hdr, 'Content-Length' => length "$content")
      if !header_exists ($hdr, 'Content-Length')
             && !ref $content && @_ == 5;
...
sub response {
   ...
   if ($no_body) { # for HEAD requests!
      my $res = $self->create_header($code, $msg, $hdr, $content);
   ...
   if (ref ($content) eq 'CODE') {
      ...
      unless ($self->{keep_alive}) {
        # no new data from client is possible when response is being made;
        # just setup this dummy reader to catch client disconnection quickly
        $self->{hdl}->on_read(sub {
            $_[0]->rbuf = '';
        });
      }

      my $response_cb = $content;

      my $writer = Edge::Manager::HTTPD::Extended::HTTPConnection::Writer->new($self);
      $self->{writer} = $writer;
      weaken($self->{writer});

      if ($code > 0) {
        my $res = $self->create_header($code, $msg, $hdr);
        $writer->write($res);
        $response_cb->($writer);
      }
      else {
        ...
        $response_cb->($writer, sub { # $code, $msg, $hdr, $content
            return unless $self_w;
            return $self_w->create_header(@_);
        });
      }
   } else {
      my $res = $self->create_header($code, $msg, $hdr, $content);
      #$res .= $content; # already written
      $self->{hdl}->push_write ($res);
      $self->response_done;
   }
}
